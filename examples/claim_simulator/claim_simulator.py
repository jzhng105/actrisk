...
class ClaimSimulator:
    def __init__(self, policies_df, num_simulations=10000, random_seed=1234):
        self.policies = policies_df
        self.num_simulations = num_simulations
        self.random_seed = random_seed
        self.claim_data = None
        self.claim_development = None

    def group_policies(self):
        self.policies['freq_bucket'] = self.policies['freq_lambda'].round(2)
        self.policies['sev_mu_bucket'] = self.policies['sev_mu'].round(2)
        self.policies['sev_sigma_bucket'] = self.policies['sev_sigma'].round(2)

        return self.policies.groupby(['freq_bucket', 'sev_mu_bucket', 'sev_sigma_bucket'])
    ...
    def simulate_claims(self):
        ...
        for group_params, group_df in grouped_policies:
            freq_param = group_params[0]
            sev_params = group_params[1:]
